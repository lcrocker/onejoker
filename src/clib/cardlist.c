 ...
 * successful and a negative OJE_ error code if not.
 */
...
int ojl_new(oj_cardlist_t *sp, int *buf, int size) {
    ...
    sp->allocation = size;
    sp->length = sp->pflags = sp->eflags = 0;
    sp->mask = 0LL;
    sp->user_info = NULL;
    sp->cards = buf;
    return size;
...
int ojl_clear(oj_cardlist_t *sp) {
    ...
    if (sp->pflags & OJF_RDONLY) return OJE_RDONLY;
...
int ojl_truncate(oj_cardlist_t *sp, int size) {
    ...
    if (size > sp->length) return OJE_BADINDEX;
...
int ojl_get(oj_cardlist_t *sp, int index) {
    assert(0 != sp && 0x10ACE0FF == sp->_johnnymoss);
    if (index >= sp->length) return OJE_BADINDEX;
...
int ojl_set(oj_cardlist_t *sp, int index, int card) {
    ...
    if (sp->pflags & OJF_UNIQUE) {
        if (ISSET(sp->mask, card)) return OJE_DUPLICATE;
        CLEAR(sp->mask, oc);
        SET(sp->mask, card);
    }
    sp->cards[index] = card;
...
int ojl_set_pflag(oj_cardlist_t *sp, int mask) {
    assert(0 != sp && 0x10ACE0FF == sp->_johnnymoss);
    if ((sp->pflags & OJF_RDONLY) && (OJF_RDONLY != mask)) return OJE_RDONLY;
 ...
 */
void *ojl_get_user_info(oj_cardlist_t *sp) { return sp->user_info; }
void ojl_set_user_info(oj_cardlist_t *sp, void *udp) { sp->user_info = udp; }
...
int ojl_append(oj_cardlist_t *sp, int card) {
    ...
    if (sp->pflags & OJF_RDONLY) return OJE_RDONLY;
    if (sp->length == sp->allocation) return OJE_FULL;
...
int ojl_extend(oj_cardlist_t *destp, oj_cardlist_t *srcp, int count) {
    ...
    if (destp->pflags & OJF_RDONLY) return OJE_RDONLY;
    if (count > (destp->allocation - destp->length)) return OJE_FULL;

    if (srcp->pflags & OJF_UNIQUE) {
        c = 0;
        for (i = 0; i < count; ++i) {
            r = ojl_append(destp, srcp->cards[i]);
            if (OJE_DUPLICATE == r) return OJE_DUPLICATE;
            if (r > 0) ++c;
        }
...
int ojl_pop(oj_cardlist_t * sp) {
    ...
    if (0 == sp->length) return OJE_BADINDEX;
    s = --sp->length;
...
int ojl_remove(oj_cardlist_t * sp, int card) {
    ...
    if (sp->pflags & OJF_RDONLY) return OJE_RDONLY;
    i = ojl_index(sp, card);
    if (OJE_NOTFOUND == i) return OJE_NOTFOUND;
...
int ojl_copy(oj_cardlist_t *destp, oj_cardlist_t *srcp) {
    ...
    if (destp->pflags & OJF_RDONLY) return OJE_RDONLY;
    if (destp->allocation < srcp->length) return OJE_FULL;
...
int ojl_fill(oj_cardlist_t *sp, int count, oj_deck_type_t dt) {
    ...
    if (sp->pflags & OJF_RDONLY) return OJE_RDONLY;
    if (count > sp->allocation) return OJE_FULL;
    if ((sp->pflags & OJF_UNIQUE) && (count > dp->length))
        return OJE_DUPLICATE;
